...
annotations_with_more_than_one_rating = []
...
for annotation in annotations_json:
    if not (key_exists_in_dict(annotation, 'score') and key_exists_in_dict(annotation['score'], 'ratedBy')):
        continue
    if len(annotation['score']['ratedBy']) > 1:
        annotations_with_more_than_one_rating.append(annotation)

voters = []
for annotation in annotations_with_more_than_one_rating:
    ratings = annotation['score']['ratedBy']
    for rating in ratings:
        if rating['withCreator'] not in voters:
            voters.append(rating['withCreator'])
print(f'Number of voters: {len(voters)}')

votes = []
for annotation in annotations_with_more_than_one_rating:
    annotation_votes = [np.nan] * len(voters)

    ratings = annotation['score']['ratedBy']
    for rating in ratings:
        for voter in voters:
            if rating['withCreator'] == voter:
                annotation_votes[voters.index(voter)] = rating['confidence']
    votes.append(annotation_votes)

# print(votes)

df = pd.DataFrame(votes)
df.T.to_csv('votes.csv')
# print(df.T)
correct_data = df.values.T.tolist()
np_correct_data = np.array(correct_data)

v_domain = [float(i) for i in list(range(101))]

print("ratio metric: %.5f" % alpha(np_correct_data, level_of_measurement='interval', value_domain=v_domain))
